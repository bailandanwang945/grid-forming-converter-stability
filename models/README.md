...
外部参照采用 MathWorks 官方 GFM Simscape 仓库固定发布 `23.2.1.4`，只读放在被 Git 忽略的 `external/mathworks-gfm-r2023b`。项目侧适配脚本位于 `experiments/external-validation/`：当前只完成 VSM、虚拟阻抗、`X/R=5` 下三个 SCR 点的有功阶跃对照，并观察到 SCR=5 工况由供应商分类函数判为失稳。它用于检验团队模型结论是否依赖自建方程，不作为硬件或论文定理确认。
