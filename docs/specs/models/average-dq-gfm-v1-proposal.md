...
6. **待完成**：三个频点的小幅正弦扰动辨识，目标为导纳幅值误差 `<1%`、相位误差 `<1°`；
...
10. **部分完成**：阻尼变化已有稳定与失稳回归；`R/X、内环带宽、nq` 的系统灵敏度与临界附近回归待补。
...
当前模型的 `Kδ≈1/X` 只是无损、平坦、单位电压特例。实现现已在同一带载工作点保持 Q–V 准稳态关系，数值计算 `Kδ=∂p/∂δ`，再组装三状态 VSM—有功测量近似。冻结校核算例中，三状态近似相对16状态模型主导模态的振荡频率与衰减率误差均小于 `5%`。该结果只支持“当前参数下低频模型抓住了主导同步模态”，不证明二者一般等价；后续仍需系统改变 `R、nq、Tm` 与内环带宽，观察误差何时扩大。
