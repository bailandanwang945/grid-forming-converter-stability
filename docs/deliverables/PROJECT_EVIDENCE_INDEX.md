...
| C11 | 自动测试与证据边界 | 当前分支 Python 270项全量通过；同一变更后的前端构建、启动器和真实浏览器通过；MATLAB 冻结候选128项通过 | [`docs/software/VERIFICATION.md`](../software/VERIFICATION.md)、[`results/test-reports/`](../../results/test-reports/)、[`backend/tests/`](../../backend/tests/)、[`tests/`](../../tests/) | `powershell.exe -File .\scripts\verify_all.ps1 -NoBootstrap` | Python、浏览器与 MATLAB 验证不同实现；分层运行记录不能冒充同一次统一入口执行 |
