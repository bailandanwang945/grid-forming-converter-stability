...
1. Python `unittest` 全套测试，当前测试集为 52 项；
...
`D:\matlab` 仅为示例，应替换为本机实际安装目录。当前仓库中可追溯的最新 MATLAB 记录为 128 Passed、0 Failed、0 Incomplete；Python 当前测试集为 52 项。两者验证不同实现，不应相加或互相替代。
...
| 解压目录大小 | 151.48 MiB |
| ZIP 大小 | 66.09 MiB |
| ZIP SHA-256 | `8f55d0cc0ca236dfa6672d00e53bb7cd3441b9e8c79ec672dbefe685cb4c62fd` |
| 工作树状态 | 脏工作树；仅用于本机验证 |
| 发布结论 | 不是正式 release |

上述哈希只标识本次本机候选，不得沿用于后续正式包。正式版本必须在干净提交上重新构建，并重新生成清单、大小和 SHA-256。

M5 发布完成至少需要保存以下证据：

- 与确定提交版本对应的发布目录、ZIP、文件清单与 SHA-256；
- 在干净提交上重建正式包，并完成打包后启动、健康检查、分析和退出清理；
- 在另一台没有 Python、Node.js、MATLAB 和项目缓存的 Windows 电脑上断网启动；
- 完成 Fig. 8 分析、独立案例导入重算、CSV/报告导出和退出清理；
- 记录操作系统、命令、结果、失败现象和修复版本。

当前本机候选证明发布技术路径可行，但不证明跨机可运行。只有干净提交重建及上述异机断网验收完成，才可把正式发布包称为“无需开发环境、可跨机运行”。许可证与第三方材料分发边界未确定前，不得对外正式发布。
