...
With these definitions, the converter’s new input–output relation becomes as in \ref{eq:pp_trasnf} and similar the transformation is applied to the network, yielding \ref{eq:pp_trasnf_net}.
\begin{equation}
...
Assuming the transformation is well-defined, input/output stability of the transformed system and the original system are equivalent. Indeed, the stability is determined by the zeros of $\det(Y_{net}(s)+Y_C(s))$. Since $\det(\mathbf{J}_C(s) +\mathbf{J}_{net}) = \det(\mathcal{E}(Y_{net}(s)+Y_{C}(s))\mathcal{F})$, the zeros of the transformed system and the original are identical. Moreover, if there are no right plane pole/zero cancellations, then
...
Nevertheless, for the small-phase criterion to be applicable, sectoriality must hold across the entire frequency range for both $J_C(s)$, and $J^{-1}_{net}(s)$. 
...
\begin{figure} [t]
    \centering
    \includegraphics[width=\linewidth]{4_Phase.pdf}
...
Ideally, at low frequencies the matrices should correspond to those in \eqref{eq:change_of_coord_1}-\eqref{eq:change_of_coord_2} (denoted from now on as $\mathcal{E}_J$, $\mathcal{F}_J$ and $\mathcal{C}_J$), while at high frequencies they should revert to the rectangular frame (i.e., $\mathcal{E}=\mathcal{F}=I$ and $\mathcal{C}=0$, denoted from now on as $\mathcal{E}_Y$, $\mathcal{F}_Y$ and $\mathcal{C}_Y$). A continuous transition between these two regimes is necessary. A trivial choice for $\mathcal{E}(s)$, and similar for $\mathcal{F}(s)$ and $\mathcal{C}(s)$, can be as in (15) using an low/high-pass filtering approach. 
...
\end{equation}
This transformation proves in general inadequate since sectoriality near the filter cut-off frequency $w_c$. 
